...
growth_trajectories = function(beta1=NULL, beta2=NULL, N=50){
  ...
  for (f in 1:N){
    ...
    for (i in 2:30){
      ...
      fish$grow(tsb, temp)
      fish$set_age(fish$age+1)
      dat[i,] = c(fish$get_state(), fish$naturalMortalityRate(temp), temp, fish$produceRecruits(0.8*tsb*1e6), 0.8*tsb) # Assuming SSB is 80% of TSB, since we are not simulating a population
    }
...
```{r}
dat_full = growth_trajectories(N=200)
```
...
$$
m(a,l_a,T) = \frac{1}{1 + e^{-\left(\frac{l_{a} - (s_{m}a + i_{m})}{d_{m}} + \beta_3(T-T_\text{ref})\right)}},
$$
...
calc_maturation_prob1 = function(a,l){
  fish$set_age(a)
  fish$set_length(l)
  p = fish$maturationProb(5.61)
  p
}
...
```{r}
dat_full %>% mutate(length_class = ave(length, cut(length, breaks=20)), temp_class = ave(temp, cut(temp, breaks=5))) %>% group_by(length_class, temp_class) %>% summarize(maturity = mean(isMature)) %>% ggplot() + geom_line(aes(y=maturity, x=length_class, group=temp_class, colour=temp_class))+scale_colour_viridis_c()+theme_classic(base_size = 20)+labs(x="Length", y="Maturity", colour="T")
```
...
Number of recruits additionally also follows a Beverton-Holt density dependence with parameter $B_{1/2}$,
...
```{r}
tibble(x = seq(0,10, length.out=100)) %>% mutate(y = 1/(1+x/0.17)) %>% 
ggplot(aes(y=y,x=x)) + geom_line(col="cyan3", size=2) + labs(x="Spawning stock biomass (Mt)", y = "Fraction of surviving recruits") + theme_classic(base_size = 20)
```
...
```{r}
dat_full %>% ggplot(aes(x=ssb, y=fec)) + geom_point(aes(col=temp)) + scale_colour_viridis_c() + labs(x="Spawning stock biomass (kT)", y = "Recruits per capita") + theme_classic(base_size=20)
```
